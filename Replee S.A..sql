...
--RESUMEN DE CLIENTES POR REGIÓN / SE USAN FUNCIONES DE FECHA PARA CALCULAR EL TIEMPO / SE ORDEN POR LOS AÑOS SOLCITADOS
--ALMACENO LA CONSULTA EN UNA VISTA LLAMADA : CLIENTES_CON_20_ANNOS_X_REGION
CREATE OR REPLACE VIEW CLIENTES_CON_20_ANNOS_X_REGION AS
SELECT
    ...
    (SELECT COUNT(*)
     FROM 
        cliente c2
     WHERE 
        c2.cod_region = r.cod_region
       AND 
        c2.fecha_inscripcion <= SYSDATE - INTERVAL '240' MONTH) AS clientes_mas_20_annos,
    (SELECT COUNT(*)
     FROM 
        cliente c3
     WHERE 
        c3.cod_region = r.cod_region) AS total_clientes
     FROM 
        region r;

--ORDENARLO DE FORMA ASCENDENTE
--Tuve que agregar el ORDER BY fuera de la vista, ya que al incluirlo dentro no me garantizaba que los resultados 
--siempre estuvieran ordenados al consultar la vista. 
SELECT * FROM CLIENTES_CON_20_ANNOS_X_REGION
ORDER BY 
    CLIENTES_MAS_20_ANNOS ASC;

--VEO EL PLAN DE EJECUCIÓN
--Al realizar la solicitud de orden ascendente fuera de la vista, permitió al motor de Oracle optimizar mejor el 
--acceso a los datos utilizando los índices disponibles (IDX_REGION e IDX_CLI_REGION). 
EXPLAIN PLAN FOR
SELECT * FROM CLIENTES_CON_20_ANNOS_X_REGION;

SELECT * FROM TABLE(DBMS_XPLAN.DISPLAY);

--SE CREAN INDICES PARA OPTIMIZAR EL ACCESO
--Intente estableciendo varias formas de establecer los índices pero no logre que el IDX_CLI_REGION accediera a la tabla cliente
--a través de un BY INDEX ROWID. Lo más cercano que encontre fue un RANGE SCAN. Bueno ambos índices tanto IDX_REGION e IDX_CLI_REGION
--acceden a la tabla cliente a través de un RANGE SCAN. Quiere decir que de cierto modo se esta optimizando, no del todo, pero algo
--se esta haciendo. Espero con ansias la retroalimentación de esto profesor para ver en que falle, de verdad que lo intente de mil
--maneras posibles.
...
--*************************************************************************2) REQUERIMIENTO *************************************
--*******************************************************************************************************************************
--TABLAS EMPLEADAS EN EL DESARROLLO DE MI EJERCICIO
select*from cuota_transac_tarjeta_cliente;
select * from tipo_transaccion_tarjeta;

--CONSULTA QUE VISUALIZA PROMEDIO DE TRANSACCIONES ORDENADOS DE MANERA ASCENDETE, UTILIZANDO LA 
--ALTERNATIVA 1: USANDO UN OPERADOR SET   "UNION ALL"
SELECT 
    TO_CHAR(CT.FECHA_VENC_CUOTA, 'DD-MM-YYYY') AS FECHA,
    LPAD(TT.COD_TPTRAN_TARJETA, 8, ' ') AS CODIGO,
    UPPER(TT.NOMBRE_TPTRAN_TARJETA) AS DESCRIPCION,
    ROUND(AVG(CT.VALOR_CUOTA), 0) AS "MONTO PROMEDIO TRANSACCION"
FROM 
    CUOTA_TRANSAC_TARJETA_CLIENTE CT
INNER JOIN 
    TRANSACCION_TARJETA_CLIENTE TC ON CT.NRO_TRANSACCION = TC.NRO_TRANSACCION
INNER JOIN 
    TIPO_TRANSACCION_TARJETA TT ON TC.COD_TPTRAN_TARJETA = TT.COD_TPTRAN_TARJETA
WHERE 
    EXTRACT(MONTH FROM CT.FECHA_VENC_CUOTA) BETWEEN 6 AND 12
GROUP BY 
    TO_CHAR(CT.FECHA_VENC_CUOTA, 'DD-MM-YYYY'), TT.COD_TPTRAN_TARJETA, TT.NOMBRE_TPTRAN_TARJETA
...
WHERE 
    EXTRACT(MONTH FROM CT.FECHA_VENC_CUOTA) BETWEEN 6 AND 12
    AND TC.NRO_TRANSACCION IS NULL
ORDER BY 
    "MONTO PROMEDIO TRANSACCION" ASC;

-- ALTERNATIVA 2: HACIENDO USO DE UNA SUBCONSULTA CUYOS DATOS SE ALMACENARAN EN UNA TABLA SELECCION_TIPO_TRANSACCION.
DESC seleccion_tipo_transaccion;
...
SELECT 
    ...
    ROUND(AVG(CT.VALOR_CUOTA), 0) AS MONTO_PROMEDIO
FROM 
    CUOTA_TRANSAC_TARJETA_CLIENTE CT
LEFT JOIN 
    TRANSACCION_TARJETA_CLIENTE TC ON CT.NRO_TRANSACCION = TC.NRO_TRANSACCION
...
WHERE 
    EXTRACT(MONTH FROM CT.FECHA_VENC_CUOTA) BETWEEN 6 AND 12
GROUP BY 
    TO_CHAR(CT.FECHA_VENC_CUOTA, 'DD-MM-YYYY'),
    TT.COD_TPTRAN_TARJETA,
    TT.NOMBRE_TPTRAN_TARJETA

COMMIT;

--CONSULTA PARA VISUALIZAR LOS DATOS ORDENADOS EN LA TABLA SELECCION_TIPO_TRANSACCION POR EL MONTO_PROMEDIO DE MENOR A MAYOR(ASC).
SELECT FECHA, COD_TIPO_TRANSAC, NOMBRE_TIPO_TRANSAC, MONTO_PROMEDIO
FROM 
    SELECCION_TIPO_TRANSACCION
ORDER BY 
    MONTO_PROMEDIO ASC;

--ACTUALIZAR EL PORCENTAJE DE LA TASA DE INTERÉS DEL TIPO DE TRANSACCIÓN EN LA TABLA TIPO_TRANSACCION_TARJETA

--VERIFICO LOS CODIGOS PRESENTES EN LA CONSULTA
SELECT 
    COD_TPTRAN_TARJETA, NOMBRE_TPTRAN_TARJETA, TASAINT_TPTRAN_TARJETA
FROM 
...
--ACTUALIZACIÓN DEL INTERES DE CODIGOS PRESENTES 101,102 Y 103 CON UN DESCUENTO DEL 1%. 
--SEÑALAR QUE LA TAZA DE INTERES ESTA ALMACENADA EN FORMATO DECIMAL, ES DECIR 0,03 EQUIVALE A 3%, 0,04 EQUIVALE A 4% 
--Y 0,14 EQUIVALE A 14%, POR ESO COMO EN LAS INSTRUCCIONES SEÑALA QUE LA DISMINUCIÓN TIENE QUE SER DE 1%, ESTO EQUIVALE
--A 0,01 EN FORMATO DECIMAL.
UPDATE 
    TIPO_TRANSACCION_TARJETA TT
SET 
    TASAINT_TPTRAN_TARJETA = TASAINT_TPTRAN_TARJETA - 0.01
WHERE EXISTS (
    SELECT 1
    FROM 
        SELECCION_TIPO_TRANSACCION STT
    WHERE 
        STT.COD_TIPO_TRANSAC = TT.COD_TPTRAN_TARJETA
);

COMMIT;

--VERIFICAR DISMINUCIONES EN LOS CODIGOS ESPECIFICOS SEÑALADOS (101, 102, 103)
SELECT 
    COD_TPTRAN_TARJETA, NOMBRE_TPTRAN_TARJETA, TASAINT_TPTRAN_TARJETA
FROM 
    TIPO_TRANSACCION_TARJETA
...
--VISUALIZACIÓN DE IMAGEN PRESENTE EN LAS INSTRUCCIONES ESPECIFICAS CON LOS NUEVOS INTERESES PARA CÓDIGOS 101,102 Y 103
SELECT * FROM  tipo_transaccion_tarjeta;
--*******************************************************************************************************************************
--*************************************************************************2) REQUERIMIENTO *************************************
--****************************************RESPUESTAS A PREGUNTAS SOLICITADAS*****************************************************
--1.- ¿Cuál es el problema que se debe resolver ? 
--El problema que se debe resolver es obtener la información relacionada con las transacciones realizadas y cuyas cuotas vencen
--entre los meses de junio y diciembre. Se debe mostrar la descripción del tipo de transacción y el cálculo del valor promedio 
--de los montos de las cuotas asociadas a cada tipo de transacción dentro del rango de fechas solicitado. Para finalmente ordenar
--dicha información referida al promedio de manera ascendente.

--2.-¿Cuál es la información significativa que necesita para resolver el problema?
--La información que necesite proviente a las siguientes tablas:
--)CUOTA_TRANSAC_TARJETA_CLIENTE, con la cual obtuve la fecha de vencimiento de las cuotas y los valores de las mismas. 
--Pudiendo así filtrar los meses solicitados y calcular el promedio de las cuotas correspondientes a cada tipo de trasacción.
--)TIPO_TRANSACCION_TARJETA, de la cual obtuve la descripción del tipo de transacción y el código asociado a cada una.
--)TRANSACCION_TARJETA_CLIENTE, esta última me sirvió como puente entre las dos anteriores,
--ya que asi pude vincular las cuotas con su tipode transacción.

--3.-¿Cuál es el propósito de la solución que se requiere ?
--El propósito es presentar la información ordenada de manera clara y precisa sobre las transacciones cuyos vencimientos están 
--dentro del segundo semestre (junio a diciembre). Mostrando a la vez la descripción del tipo de transacción, los códigos asociados 
--y el promedio de los montos de las transacciones. Cuya información después le servirá a Replee S.A. para actualizar las tasas 
--de interés en la tabla TIPO_TRANSACCION_TARJETA. La idea es no solo visualizar la información, sino también realizar
--cambios en la base de datos según las condiciones del negocio.

--4.-Detalle los pasos, en el lenguaje natural, necesarios para construir la alternativa que usa SUBCONSULTA.

--|1) Identificar las cuotas que vencen entre junio y diciembre. Esto lo hice filtrando las fechas de la tabla 
--CUOTA_TRANSAC_TARJETA_CLIENTE a través del filtro `EXTRACT(MONTH FROM ...)`.
--|2) Calcular el promedio de los montos. Aquí, la subconsulta calculó el promedio de las cuotas (`AVG(...)`) para cada
--tipo de transacción, integrándolo directamente en la consulta.
--|3) Vinculación de las tablas. Uní la tabla `CUOTA_TRANSAC_TARJETA_CLIENTE` con `TRANSACCION_TARJETA_CLIENTE` usando un 
--`LEFT JOIN` y luego con `TIPO_TRANSACCION_TARJETA` también con un `LEFT JOIN`, para obtener la descripción y el código 
--del tipo de transacción.
--|4) Insertar los resultados requeridos por la empresa. Los guardé directamente en la tabla `SELECCION_TIPO_TRANSACCION` 
--a través de un `INSERT`.
--|5) Ordenar los datos de manera ascendente basándome en la información del promedio, usando `ASC`.


--5.-Detalle los pasos, en lenguaje natural, necesarios para construir la alternativa que usa OPERADOR SET

--|1) Identificar las cuotas que vencen entre junio y diciembre. Esto lo hice filtrando las fechas en la tabla 
--`CUOTA_TRANSAC_TARJETA_CLIENTE` mediante el filtro `EXTRACT(MONTH FROM ...)`.
--|2) Calcular el promedio de los montos. La primera consulta calculó el promedio de las cuotas (`AVG(CT.VALOR_CUOTA)`) para 
--cada tipo de transacción, agrupando los datos por fecha, código y descripción.
--|3) Asignar un promedio de 0 a transacciones sin cuotas válidas. Esto se realizó en la segunda consulta, identificando las 
--transacciones sin cuotas válidas mediante un `LEFT JOIN` y asignando un promedio de `0`.
--|4) Combinar ambas consultas. Usé el operador `UNION ALL` para combinar los resultados de las dos consultas, asegurándome 
--de incluir tanto las transacciones con promedio calculado como las que no tenían cuotas válidas.
--|5) Ordenar los resultados. Los resultados combinados se ordenaron de forma ascendente según el promedio de montos, 
--utilizando la cláusula `ORDER BY "MONTO PROMEDIO TRANSACCION" ASC`.
